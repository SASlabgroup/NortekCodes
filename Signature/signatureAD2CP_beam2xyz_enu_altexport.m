function [ Data, Config, T_beam2xyz ] = signatureAD2CP_beam2xyz_enu_altexport( Data, Config, mode, twoZs, declination )
% J. Thomson editted 11/2019 to make compatible with exports from
% SigDeployment, rather than from MIDAS
...
activeBeams = Data.( [dataModeWord '_BeamToChannelMapping'] )( 1, : );
...
xAllCells = zeros( length( Data.( [ dataModeWord '_Time' ] ) ), Config.( [ad2cpstr  configModeWord '_NCells' ] ) );
yAllCells = zeros( length( Data.( [ dataModeWord '_Time' ] ) ), Config.( [ad2cpstr  configModeWord '_NCells' ] ) );
zAllCells = zeros( length( Data.( [ dataModeWord '_Time' ] ) ), Config.( [ad2cpstr  configModeWord '_NCells' ] ) );
if twoZs == 1
	z2AllCells = zeros( length( Data.( [ dataModeWord '_Time' ] ) ), Config.( [ad2cpstr  configModeWord '_NCells' ] ) );
end

xyz = zeros( size( T_beam2xyz, 2 ), length( Data.( [ dataModeWord '_Time' ] ) ) );
beam = zeros( size( T_beam2xyz, 2 ), length( Data.( [ dataModeWord '_Time' ] ) ) );
for nCell = 1:Config.( [ad2cpstr  configModeWord '_NCells' ] )
	for i = 1:numberOfBeams
		beam( i, : ) = Data.( [ dataModeWord '_VelBeam' num2str( 1 + Data.( [ dataModeWord '_BeamToChannelMapping' ] )( 1, i ) ) ] )( :, nCell )';
	end
	xyz = T_beam2xyz * beam;
...
K = 3;
EAllCells = zeros( length( Data.( [dataModeWord  '_Time' ] ) ), Config.( [ad2cpstr   configModeWord '_NCells' ] ) );
NAllCells = zeros( length( Data.( [dataModeWord  '_Time' ] ) ), Config.( [ad2cpstr   configModeWord '_NCells' ] ) );
UAllCells = zeros( length( Data.( [dataModeWord  '_Time' ] ) ), Config.( [ad2cpstr   configModeWord '_NCells' ] ) );
if twoZs == 1
	U2AllCells = zeros( length( Data.( [dataModeWord  '_Time' ] ) ), Config.( [ad2cpstr  configModeWord '_NCells' ] ) );
   K = 4;
end
