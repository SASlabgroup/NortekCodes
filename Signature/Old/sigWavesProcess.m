...
%
%   S.D.Brenner, 2019
%   J. Thomson, Nov 2019 switched to matlab field names from other export
...
hybridspectra = false;          % option to make a hybrid spectrum with pressue and AST


%% Extract simple values from Data structure

time = Data.Burst_Time(1);
...
if isfield(Data,'Burst_AltimeterAST_corrected')
    ast = Data.Burst_AltimeterDistanceAST_Corrected;
    le = Data.Burst_AltimeterDistanceLE_Corrected;
else
    ast = Data.Burst_AltimeterDistanceAST;
    le = Data.Burst_AltimeterDistanceLE;
end

depth = mean(pres);

% Calculate sampling frequency
ts = mean(diff(Data.Burst_Time)) * 86400; % sampling period  [sec]
...
blanking = 17; % need to get these from Config structure
binsize = 1.7; % need to get these from Config structure
ranges = blanking + double(Data.Burst_NCells) .* [1:binsize:length(Data.Burst_NCells)];
rDepths = depth - ranges;
targetDepth = 0.1*depth;
[~,minInd] = min( abs(rDepths-targetDepth) );

% Extract values
v = -Data.Burst_VelEast(:,minInd);
u = Data.Burst_VelNorth(:,minInd);
...
%figure(10), clf, plot(ast), drawnow
...
for ki = 1:length(f), k(ki) = wavenumber( f(ki), waterdepth); end
